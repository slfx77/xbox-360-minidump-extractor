...
public sealed partial class HexViewerControl : UserControl, IDisposable
{
    ...
    {
        _minimapRenderer = new HexMinimapRenderer(MinimapCanvas, ViewportIndicator, MinimapScrollViewer, FindRegionForOffset);
        ...
        DispatcherQueue.TryEnqueue(() =>
        {
            ...
            if (_analysisResult != null) { RenderVisibleRows(); RenderMinimap(); UpdateMinimapViewport(); }
        });
    }
    ...
    private void UpdateVisibleRowCount()
    {
        ...
        _visibleRows = HexDisplayArea.ActualHeight - 8 > 0 ? (int)Math.Floor((HexDisplayArea.ActualHeight - 8) / _rowHeight) : 30;
        VirtualScrollBar.StepFrequency = 1;
    }

    private void CleanupMemoryMapping() { _accessor?.Dispose(); _accessor = null; _mmf?.Dispose(); _mmf = null; }

    public void Clear()
    {
        CleanupMemoryMapping();
        _filePath = null; _analysisResult = null; _fileSize = 0;
        _fileRegions.Clear(); _currentTopRow = 0; _totalRows = 0;
        _rowRenderer?.Clear();
    ...
    public void LoadData(string filePath, AnalysisResult analysisResult)
    {
        ...
        catch (Exception ex) { Debug.WriteLine($"[HexViewer] Memory mapping failed: {ex.Message}"); }
    ...
    private void BuildFileRegions()
    {
        ...
        foreach (var file in sortedFiles)
        {
            ...
            if (occupiedRanges.Any(r => start < r.End && end > r.Start && r.Priority <= priority)) continue;

            _fileRegions.Add(new FileRegion
            {
                Start = start, End = end,
                TypeName = file.FileType,
                Color = FileTypeColors.GetColor(FileTypeColors.NormalizeTypeName(file.FileType))
            });
    ...
    {
        var isCtrl = InputKeyboardSource.GetKeyStateForCurrentThread(VirtualKey.Control).HasFlag(CoreVirtualKeyStates.Down);
        var delta = e.GetCurrentPoint(HexDisplayArea).Properties.MouseWheelDelta;
        ScrollByRows(delta > 0 ? -(isCtrl ? 10 : 1) : (isCtrl ? 10 : 1));
        e.Handled = true;
    }
    ...
    private void HexViewerControl_KeyDown(object sender, KeyRoutedEventArgs e)
    {
        switch (e.Key)
        {
            case VirtualKey.PageUp: ScrollByRows(-_visibleRows); e.Handled = true; break;
            case VirtualKey.PageDown: ScrollByRows(_visibleRows); e.Handled = true; break;
            case VirtualKey.Home: ScrollToRow(0); e.Handled = true; break;
            case VirtualKey.End: ScrollToRow(Math.Max(0, _totalRows - _visibleRows)); e.Handled = true; break;
            case VirtualKey.Up: ScrollByRows(-1); e.Handled = true; break;
            case VirtualKey.Down: ScrollByRows(1); e.Handled = true; break;
        }
    }
    ...
    {
        if (e.Key == VirtualKey.C && InputKeyboardSource.GetKeyStateForCurrentThread(VirtualKey.Control).HasFlag(CoreVirtualKeyStates.Down) && sender is TextBlock tb)
        { e.Handled = true; CopySelectedText(tb); }
    }
    ...
    private static void CopySelectedText(TextBlock textBlock)
    {
        ...
        if (!string.IsNullOrEmpty(text))
            ClipboardHelper.CopyText(textBlock.Name.Contains("Ascii") ? text.Replace("\n", "").Replace("\r", "") : text);
    }
    ...
    private void ScrollByRows(long delta)
    {
        ...
        { _currentTopRow = newValue; VirtualScrollBar.Value = newValue; RenderVisibleRows(); UpdateMinimapViewport(); UpdatePositionIndicator(); }
    }
    ...
    private void RenderVisibleRows()
    {
        ...
        if (bytesToRead <= 0) { _rowRenderer.Clear(); return; }
    ...
    private void ReadBytes(long offset, byte[] buffer)
    {
        ...
        else if (_filePath != null) { using var fs = new FileStream(_filePath, FileMode.Open, FileAccess.Read, FileShare.Read); fs.Seek(offset, SeekOrigin.Begin); fs.ReadExactly(buffer); }
    }
    ...
    private FileRegion? FindRegionForOffset(long offset)
    {
        ...
        while (left <= right)
        {
            ...
            if (region.Start > offset) right = mid - 1; else left = mid + 1;
        }
        return null;
    }
    ...
    private void MinimapContainer_SizeChanged(object sender, SizeChangedEventArgs e)
    {
        ...
        { _lastMinimapContainerHeight = e.NewSize.Height; if (_analysisResult != null) { RenderMinimap(); UpdateMinimapViewport(); } }
    }

    private void MinimapZoomSlider_ValueChanged(object sender, RangeBaseValueChangedEventArgs e)
    {
        if (_minimapRenderer != null) { _minimapRenderer.Zoom = e.NewValue; if (_analysisResult != null) { RenderMinimap(); UpdateMinimapViewport(); } }
    }

    private void RenderMinimap() => _minimapRenderer?.Render(_fileSize, MinimapContainer.ActualWidth - 8, MinimapContainer.ActualHeight - 8);
    private void UpdateMinimapViewport() => _minimapRenderer?.UpdateViewport(_fileSize, _totalRows, _currentTopRow, _visibleRows, BytesPerRow);

    private void Minimap_PointerPressed(object sender, PointerRoutedEventArgs e)
    { _minimapRenderer?.HandlePointerPressed(e); NavigateToMinimapPosition(e.GetCurrentPoint(MinimapCanvas).Position.Y); }

    private void Minimap_PointerMoved(object sender, PointerRoutedEventArgs e)
    { if (_minimapRenderer?.IsDragging == true) NavigateToMinimapPosition(e.GetCurrentPoint(MinimapCanvas).Position.Y); }

    private void Minimap_PointerReleased(object sender, PointerRoutedEventArgs e) => _minimapRenderer?.HandlePointerReleased(e);
    ...
    private void NavigateToMinimapPosition(double y)
    {
        ...
        if (targetRow.HasValue) { _currentTopRow = targetRow.Value; VirtualScrollBar.Value = targetRow.Value; RenderVisibleRows(); UpdateMinimapViewport(); }
    }
    #endregion
}
