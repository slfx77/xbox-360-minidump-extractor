...
public sealed class BikFormat : FileFormatBase
{
    ...
    public override ParseResult? Parse(ReadOnlySpan<byte> data, int offset = 0)
    {
        ...
        if (!magic.SequenceEqual("BIK"u8)) return null;

        // Check version byte (i, h, b, etc.)
        var versionByte = (char)data[offset + 3];
        if (versionByte < 'a' || versionByte > 'z') return null;
        ...
        try
        {
            ...
            if (frameCount == 0 || frameCount > 1000000) return null;
            ...
            if (largestFrameSize > fileSize) return null;
